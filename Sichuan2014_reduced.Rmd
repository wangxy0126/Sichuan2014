...
# originally wrote this code to examine only the forest habitats, but then i decided to include cropland too.  but i have not changed the names of the objects
...
communityB_forest <- communityB_forest[, n_communityB_forest > 1] # remove zerotons and singletons (species that appear in 0 or 1 row only)
communityB_forest.mvb <- mvabund(communityB_forest) 

nboot <- 299 # set to 299 for debugging
...
anova(mod_forest_BBECJCMFNF.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # p = 0.003 for nBoot = 299.  The Habitat predictor has a sig effect
...
anova(mod_forest_BBECJCMFNF.mvb, mod_forest_BBECJC_MFNF.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # weakly sig diff btw the two models (p=0.013 with nBoot = 299 and all non singleton OTUs). Suggests weak or no diff btw MF and NF, after correction for multiple tests.
...
anova(mod_forest_BBECJC_MFNF.mvb, mod_forest_ECJC_BBMFNF.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.017 with nBoot = 299 and all 746 OTUs), so do not combine BB with MFNF
...
anova(mod_forest_BBECJC_MFNF.mvb, mod_forest_ECBB_MFNFJC.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all 746 OTUs). so do not combine JC with MFNF
...
anova(mod_forest_BBECJC_MFNF.mvb, mod_forest_BBJC_MFNFEC.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all 746 OTUs). so do not combine EC with MFNF. Use habitatN$HabitatMFNF as the forest predictor


# combine MF, NF, and CL
habitatN_forest$HabitatMFNFCL <- fct_collapse(habitatN_forest$HabitatMFNF, MFNFCL = c("MFNF", "CL"))
fct_count(habitatN_forest$HabitatMFNFCL)

mod_forest_ECBBJC_MFNFCL.mvb <- manyglm(communityB_forest.mvb ~ HabitatMFNFCL, data = habitatN_forest, family = binomial("cloglog"))
plot(mod_forest_ECBBJC_MFNFCL.mvb)
anova(mod_forest_BBECJC_MFNF.mvb, mod_forest_ECBBJC_MFNFCL.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all 746 OTUs). so do not combine EC with MFNF. Use habitatN$HabitatMFNF as the forest predictor


# START HERE:  do a correction for multiple p-values
