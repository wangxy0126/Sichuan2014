...
use cross-validation for manyglm model selection
rewrite anova.manyglm to run in parallel, or use manyany, which has a built-in parallel option



- mvabund::traitglm with no trait matrix (= LASSO-GLM, method="cv.glm1path", slidesday4:16.12)   # this is likely the most mature way forward, given that mvabund has been in development longer than boral has been.  i might have this wrong, but so far, it looks like a cross validation method, with shrinkage, to estimate environmental coefficients, one for each species.  
- boral::boral with ssvs   # this seems quite new, and might not be ready for large datasets.  it might also choke on our dataset
- SpeciesMix  # a different approach:  to cluster the OTUs into archetypal Groups that respond similarly to the environment.  after doing this, it would be interesting to look at the taxonomies.  we don't have trait data, which is a shame.  
...
Trying another way to test for sig difference between forests. Create a forest-only dataset, then test for a habitat effect with increasingly collapsed levels, testing for differences with anova between two models
```{r}
# MF vs BB
...
communityB_forest.mvb <- communityB_forest[ , 1:350] %>% mvabund() # partial dataset for debugging
# communityB_MFBB.mvb <- communityB_MFBB %>% mvabund() # full dataset
nboot <- 100 # set to 100 for debugging
# nboot <- 999 # set to 999 for publication

mod_forest_BBECJC.mvb <- manyglm(communityB_forest.mvb ~ Habitat, data = habitatN_forest, family = binomial("cloglog"))
plot(modMFBB1.mvb)
anova(modMFBB1.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # BB is sig diff from MF
...
boral:  Testing different errors
...
X <- scale(habitatN$Altitude, scale = FALSE)

commB.fit.b3.X <- boral(communityB, X = X, family = "binomial", num.lv = 2, row.eff = "fixed", mcmc.control = mcmc.control, calc.ics = TRUE, save.model = TRUE)
