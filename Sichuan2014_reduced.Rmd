...
#### Negative binomial 
comm.fit.nb3 <- boral(community, family = "negative.binomial", num.lv = 3, row.eff = "fixed", calc.ics = FALSE, mcmc.control = mcmc.control) 
...
par(mfrow = c(1,2))
lvsplot(comm.fit.nb3, biplot=FALSE)
plot(lv2 ~ lv1, data = latvarscomm.fit.nb3)
par(mfrow = c(1,1))
...
#### Poisson
commB.fit.pb3 <- boral(communityB, family = "poisson", num.lv = 3, row.eff = "fixed", calc.ics = FALSE, mcmc.control = mcmc.control)
...
commB.fit.pb3.4 <- boral(communityB, family = "poisson", num.lv = 3, row.eff = "fixed", calc.ics = FALSE, mcmc.control = mcmc.control4)  # high iterations
summary(commB.fit.pb3.4)
par(mfrow = c(2,2))
plot(commB.fit.pb3.4) ## Plots used in residual analysis, 
par(mfrow = c(1,1))
lvsplot(commB.fit.pb3.4)
```
