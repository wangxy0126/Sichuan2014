...
# set up model
commB.fit.b3.4.none <- boral(communityB, family = "binomial", num.lv = 2, row.eff = "random", mcmc.control = mcmc.control4, save.model = TRUE); beep(sound = 8)
...
commB.fit.b3.4.none.ord <- lvsplot(commB.fit.b3.4, biplot=FALSE, col = as.numeric(habitatN$Habitat), return.vals = TRUE)
...
# set up MCMC parameters
mcmc.control <- list(n.burnin = 300, n.iteration = 1000, n.thin = 30)
# mcmc.control4 <- list(n.burnin = 10000, n.iteration = 40000, n.thin = 30)
# communityB <- communityB[, 1:150]

# set up predictors
habitat.ssvs <- habitatN %>% dplyr::select(Elevation_m, Simpson_evenness_index)
habitat.ssvs$Elevation_m <- scale(habitatN$Elevation_m, scale = FALSE)
habitat.ssvs$Simpson_evenness_index <- scale(habitatN$Simpson_evenness_index, scale = FALSE)
X <- habitat.ssvs

# set up model.  ssvs.index determines which environmental predictors i will estimate the probability of effect. I can do this for all species as a group or for each species individually.

ssvsindex <- c(0, 0) # any ssvs.index integer = 0 (e.g. c(0, 0)) will estimate the probability of that environmental predictor being important for each species
...
res.cors <- get.residual.cor(commB.fit.b3.4.none); beep(1) # from unconstrained boral ordination
...
### First mvabund task is to test for significant differences among forest types within the *habitat* predictor. 
Test for a habitat effect with increasingly collapsed levels, testing for sig differences between nested models. Outcome:  a model with MF and NF combined into one level is not sig different from one with MF and NF separate.  But all other forest types (EC, JC, BB) are sig different from the MFNF level. 

```{r combine habitat levels}
...
nboot <- 299 # set to 20 for debugging (~7.5 - 11 mins for 299)
...
anova(mod_BBCLECJCMFNF.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # p = 0.003 for nBoot = 299, 11 mins.  The Habitat predictor has a sig effect
...
anova(mod_BBCLECJCMFNF.mvb, mod_CLBBECJC_MFNF.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # weakly sig diff btw the two models (p=0.007 with nBoot = 299 and all non-singleton OTUs). Suggests weak diff btw MF and NF, after correction for multiple tests.

# combine MF, NF, and BB
habitatN$HabitatMFNFBB <- fct_collapse(habitatN$Habitat, MFNFBB = c("MF", "NF", "BB"))
fct_count(habitatN$HabitatMFNFBB)

mod_CLECJC_MFNFBB.mvb <- manyglm(communityB.mvb ~ HabitatMFNFBB, data = habitatN, family = binomial("cloglog"))
plot(mod_CLECJC_MFNFBB.mvb)
anova(mod_CLBBECJC_MFNF.mvb, mod_CLECJC_MFNFBB.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs), so do not combine BB with MFNF



# combine MF, NF, and JC
habitatN$HabitatMFNFJC <- fct_collapse(habitatN$Habitat, MFNFJC = c("MF", "NF", "JC"))
fct_count(habitatN$HabitatMFNFJC)

mod_CLECBB_MFNFJC.mvb <- manyglm(communityB.mvb ~ HabitatMFNFJC, data = habitatN, family = binomial("cloglog"))
plot(mod_CLECBB_MFNFJC.mvb)
anova(mod_CLBBECJC_MFNF.mvb, mod_CLECBB_MFNFJC.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine JC with MFNF

# combine MF, NF, and EC
habitatN$HabitatMFNFEC <- fct_collapse(habitatN$Habitat, MFNFEC = c("MF", "NF", "EC"))
fct_count(habitatN$HabitatMFNFEC)

mod_CLBBJC_MFNFEC.mvb <- manyglm(communityB.mvb ~ HabitatMFNFEC, data = habitatN, family = binomial("cloglog"))
plot(mod_CLBBJC_MFNFEC.mvb)
anova(mod_CLBBECJC_MFNF.mvb, mod_CLBBJC_MFNFEC.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine EC with MFNF. Use habitatN$HabitatMFNF as the forest predictor


# combine MF, NF, and CL
habitatN$HabitatMFNFCL <- fct_collapse(habitatN$Habitat, MFNFCL = c("MF", "NF", "CL"))
fct_count(habitatN$HabitatMFNFCL)

mod_ECBBJC_MFNFCL.mvb <- manyglm(communityB.mvb ~ HabitatMFNFCL, data = habitatN, family = binomial("cloglog"))
plot(mod_ECBBJC_MFNFCL.mvb)
anova(mod_CLBBECJC_MFNF.mvb, mod_ECBBJC_MFNFCL.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine EC with MFNF. Use habitatN$HabitatMFNF as the forest predictor


# do a correction for multiple p-values
pvalues <- c(0.007, .003, .003, .003, .003)
# pvalues.corr.fdr<-p.adjust(pvalues, method = "fdr", n = length(pvalues))
pvalues.corr.fdr<-p.adjust(pvalues, method = "fdr", n = 30) # 30 possible pairwise comparisons between all 6 forest types:  (5*6)/2 = 30 
pvalues.corr.fdr
...
anova(mod_forest_all4.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # HabitatMFNF, Elevation_m, and weather_value are all highly significant (all non-singleton OTUs).  Simpson evenness index is not sig
...
# communityB_forest <- communityB_forest[ , 1:150]  # comment away if i want to use the whole dataset.  this is to produce a partial dataset for debugging
...
```{r NF vs BB, CL, EC, JC}
# NF vs BB 
communityB_NFBB <- communityB %>% dplyr::filter(habitatN$Habitat %in% c("BB", "NF"))
habitatN_NFBB <- data.frame(habitatN.mvb[c(1:3,74)]) %>% dplyr::filter(habitatN$Habitat %in% c("BB", "NF")) # predictors can be changed
...
# NF vs CL
communityB_NFCL <- communityB %>% dplyr::filter(habitatN$Habitat %in% c("CL", "NF"))
habitatN_NFCL <- data.frame(habitatN.mvb[c(1:3,74)]) %>% dplyr::filter(habitatN$Habitat %in% c("CL", "NF")) # predictors can be changed
...
# NF vs EC
communityB_NFEC <- communityB %>% dplyr::filter(habitatN$Habitat %in% c("EC", "NF"))
habitatN_NFEC <- data.frame(habitatN.mvb[c(1:3,74)]) %>% dplyr::filter(habitatN$Habitat %in% c("EC", "NF")) # predictors can be changed
...
# NF vs JC
communityB_NFJC <- communityB %>% dplyr::filter(habitatN$Habitat %in% c("JC", "NF"))
habitatN_NFJC <- data.frame(habitatN.mvb[c(1:3,74)]) %>% dplyr::filter(habitatN$Habitat %in% c("JC", "NF")) # predictors can be changed
...
```{r}
communityBforest <- communityB %>% dplyr::filter(!(habitatN$Habitat %in% c("CL")))
...
habitatNforest <- habitatN %>% dplyr::filter(!(Habitat %in% c("CL")))
...
# 
JCNF <- communityBbetapart %>% dplyr::filter(habitatNforest$Habitat %in% c("JC", "NF")) %>% column_to_rownames(var="Site")
BBNF <- communityBbetapart %>% dplyr::filter(habitatNforest$Habitat %in% c("BB", "NF")) %>% column_to_rownames(var="Site")
ECNF <- communityBbetapart %>% dplyr::filter(habitatNforest$Habitat %in% c("EC", "NF")) %>% column_to_rownames(var="Site")
MFNF <- communityBbetapart %>% dplyr::filter(habitatNforest$Habitat %in% c("MF", "NF")) %>% column_to_rownames(var="Site")
...
# comparing to MF
JCMF <- communityBbetapart %>% dplyr::filter(habitatNforest$Habitat %in% c("JC", "MF")) %>% column_to_rownames(var="Site")
BBMF <- communityBbetapart %>% dplyr::filter(habitatNforest$Habitat %in% c("BB", "MF")) %>% column_to_rownames(var="Site")
ECMF <- communityBbetapart %>% dplyr::filter(habitatNforest$Habitat %in% c("EC", "MF")) %>% column_to_rownames(var="Site")
