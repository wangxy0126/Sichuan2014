...
habitat.ssvs$x20130817b7sd <- scale(habitatN$x20130817b7sd, scale = FALSE)
# set up categorical predictors. Factors seem to cause boral to fail
# habitat.ssvs <- habitatN %>% dplyr::select(Habitat)
...
# Gp1 Gp2 
#   0   0 
# When i ask for a group effect ssvs.index = 1:ncol(X). Neither Elevation_m nor Simpson_evenness_index are estimated to have an effect

summary(commB.fit.b3.X$ssvs.indcoefs.mean)
 #  Elevation_m       Simpson_evenness_index
 # Min.   :0.000000   Min.   :0.0000        
 # 1st Qu.:0.000000   1st Qu.:0.2083        
 # Median :0.000000   Median :0.5000        
 # Mean   :0.001979   Mean   :0.4765        
 # 3rd Qu.:0.000000   3rd Qu.:0.7083        
 # Max.   :0.166667   Max.   :1.0000        
...
## mvabund
Because boral ssvs does not seem to be able to use factor predictors, I am using mvabund to test whether MF and NF are significantly different from each other and from the other habitats:  BB, EC, JC, and CL.  I do this by creating two models, one with all 6 levels and one with NF/MF combined with one of the other two habitats. I run mvabund on both models and use anova to compare the two models. If significantly different, then the two habitat types are significantly different. Finally, i adjust the p-values for 30 possible pairwise comparisons (6 * 5)/2. 
...
nboot <- 499 # set to 20 for debugging (~7.5 - 11 mins for 299)
...
anova(mod_BBCLECJCMFNF.mvb, mod_CLECJCMF_NFBB.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs), so do not combine BB with MFNF
...
anova(mod_BBCLECJCMFNF.mvb, mod_CLECBBMF_NFJC.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine JC with MFNF
...
anova(mod_BBCLECJCMFNF.mvb, mod_CLBBJCMF_NFEC.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine EC with MFNF. Use habitatN$HabitatMFNF as the forest predictor
...
anova(mod_BBCLECJCMFNF.mvb, mod_ECBBJCMF_NFCL.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine EC with MFNF. Use habitatN$HabitatMFNF as the forest predictor
...
pvalues <- c(0.007, .002, .002, .002, .002)
...
nboot <- 20 # set to 20 for debugging (~7.5 - 11 mins for 299)
# nboot <- 999 # set to 999 for publication
...
anova(mod_BBCLECJCMFNF.mvb, mod_CLECJCNF_MFBB.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs), so do not combine BB with MFNF
...
anova(mod_BBCLECJCMFNF.mvb, mod_CLECBBNF_MFJC.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine JC with MFNF
...
anova(mod_BBCLECJCMFNF.mvb, mod_CLBBJCNF_MFEC.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine EC with MFNF. Use habitatN$HabitatMFNF as the forest predictor
...
anova(mod_BBCLECJCMFNF.mvb, mod_ECBBJCNF_MFCL.mvb, cor.type = "shrink", test = "score", show.time = "all", nBoot = nboot)  # sig diff btw the two models (p=0.003 with nBoot = 299 and all non-singleton OTUs). so do not combine EC with MFNF. Use habitatN$HabitatMFNF as the forest predictor
