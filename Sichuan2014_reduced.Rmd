...
# no nestedness
ALL.dist <- communityBbetapart %>% column_to_rownames(var="Site") %>% beta.multi(index.family="jac")
# $beta.JTU
# [1] 0.9856346
# $beta.JNE
# [1] 0.004771889
...
# pairwise beta diversities for plotting
...
mcmc.control4 <- list(n.burnin = 10000, n.iteration = 40000, n.thin = 30) # if i want to set a seed
...
saveRDS(commB.fit.b3.4.none, "analysis/boral_commB.fit.b3.4.none.RDS")
# commB.fit.b3.4.none <- readRDS("analysis/boral_commB.fit.b3.4.none.RDS") # to restore
...
Formally test the importance of environmental predictors to community analysis. 
...
ssvsindex <- rep(0, ncol(X)) # an ssvs.index integer = 0 (e.g. c(0, 0)) will estimate the probability of that environmental predictor being important for *each* species
# OR
...
# OR
ssvsindex <- rep(-1, ncol(X)) # an ssvs.index integer = -1 will *not* estimate probability of environmental predictors.  This is for when i want to generate coefsplot
...
# set up model  
# NOTE:  change mcmc.control to mcmc.control4 to carry out a long run
commB.fit.b3.X <- boral(communityB, X = X, family = "binomial", num.lv = 2, row.eff = "random", save.model = TRUE, calc.ics = FALSE, mcmc.control = mcmc.control4, prior.control = set.prior); beep(1)

saveRDS(commB.fit.b3.X, "analysis/commB.fit.b3.X.RDS") # 
...
## mvabund
Testing whether i can combine levels within habitatN$habitat.  I use mvabund to test whether MF and NF are significantly different from each other and from the other habitats:  BB, EC, JC, and CL.  I do this by creating two models, one with all 6 levels and one with NF|MF combined with one of the other two habitats. I run mvabund on both models and use anova to compare the two models. If significantly different, then the two habitat types are significantly different. Finally, i conservatively adjust the p-values for all 30 possible pairwise comparisons (6 * 5)/2. 
...
pvalues.corr.fdr<-p.adjust(pvalues, method = "fdr", n = 15) 
pvalues.corr.fdr
