...
#### Ordinal  # does not run, need to try running overnight
...
Poisson and Binomial to produce the best residuals. Re-run with more iterations
```{r boral high iterations, cache=TRUE}
...
par(mfrow = c(1,2))
lvsplot(commB.fit.pb3.4, ind.spp = 5, col = as.numeric(habitatN$Habitat), pch = 16) # ind.spp = 5:  show top 5 most important indicator species
...
#### Binomial
commB.fit.b3.4 <- boral(communityB, family = "binomial", num.lv = 2, row.eff = "fixed", mcmc.control = mcmc.control4)
summary(commB.fit.b3.4)
par(mfrow = c(2,2))
plot(commB.fit.b3.4) ## Plots used in residual analysis, 
par(mfrow = c(1,1))
par(mfrow = c(1,2))
lvsplot(commB.fit.b3.4, biplot=FALSE)
